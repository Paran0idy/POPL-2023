...
let rec subst (x, s, t) =
    ...
    | False -> raise NoRule
    | If (t1, t2, t3) -> 
        If (subst(x, s, t1), subst(x, s, t2), subst(x, s, t3))
    | Var y ->
        if x = y
          then s
          else t    
...
let rec eval t =
    match t with
      ...
      | App (Abs (x, ty11, t12), v2) ->
            if isValue v2
            then subst (x, v2, t12)              (* E-APPABS *)
            else App (Abs (x, ty11, t12), eval v2)  

      | App (t1, t2) ->
            if isValue t1
            then App (t1, eval t2)
            else App (eval t1, t2)          (* E-APP2/E-APP1 *)
...
let rec pp t =
    match t with
      ...
      | Abs (x, ty, t) ->
          (printf "\\lambda ";
           print_string x; printf ":";
           print_string (Type.toString ty);
           printf ".";
           printf "("; pp t; printf ")";
          )
